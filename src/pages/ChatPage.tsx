...
export const ChatPage: React.FC = () => {
  ...
  const activeRoom = conversations.find((c) => c.id === activeId) || conversations[0];
  useEffect(() => {
    if (activeRoom) {
      join(activeRoom.id);
    ...
    <div className="flex h-full min-h-0 w-full bg-gray-100">
      ...
      <ChatSidebar
        rooms={conversations.map((c) => {
          ...
          if (!isGroup) {
            const partMap = useChatStore.getState().participants[c.id] || {};
          ...
          return {
            ...
            isGroup,
            members,
            lastMessage: { id: '', content: (lastMsg[c.id]?.content || ''), createdAt: (lastMsg[c.id]?.createdAt || ''), sender: { id: '', name: '' }, roomId: c.id },
          };
        })}
